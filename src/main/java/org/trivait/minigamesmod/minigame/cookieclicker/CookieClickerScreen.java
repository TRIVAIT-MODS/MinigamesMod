...
public class CookieClickerScreen extends Screen {
    ...
    private static final Identifier BACKGROUND_TEXTURE = Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "textures/minigame/cookie_clicker/background.png");
    ...
    protected void init() {
        ...
        Button restartButton = SpriteIconButton.builder(Component.empty(), b -> clearGame(), true)
        ...
        restartButton.setRectangle(20, 20, 10, 35);

        addRenderableWidget(returnButton);
        addRenderableWidget(new ConfigButton(10, 60, minigame));
        ...
        addRenderableWidget(new ShopWidget(gameX + gameWidth - upgradePanelWidth + 1, gameY + 30, upgradePanelWidth - 1, gameHeight - 30, this));

        playerName = Minecraft.getInstance().getGameProfile().name();
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.centeredText(tr, String.format(Component.translatable("minigame.cookieclicker.bakery").getString(), playerName), gameX + (cookiePanelWidth / 2), gameY + 20 + 1 - 2, -1);
        ...
        context.centeredText(tr, Component.translatable("minigame.cookieclicker.cookies").append(cookiesString), 0, 0, -1);
        matrices.popMatrix();
        if (CPS != 0) {
            matrices.pushMatrix();
            matrices.translate(gameX + (cookiePanelWidth / 2), gameY + 58);
            matrices.scale(0.7f, 0.7f);
            context.centeredText(tr, Component.translatable("minigame.cookieclicker.cookiesPerSecond").append(cookiesToString(CPS)), 0, 0, -1);
            matrices.popMatrix();
        }
    }
    ...
    public void extractBackground(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.horizontalLine(gameX, gameX + gameWidth, gameY, -1);
        context.horizontalLine(gameX, gameX + gameWidth, gameY + gameHeight, -1);
        context.verticalLine(gameX, gameY, gameY + gameHeight, -1);
        context.verticalLine(gameX + gameWidth, gameY, gameY + gameHeight, -1);

        context.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND_TEXTURE, gameX + 1, gameY + 1, 0, 0, gameWidth - 1, gameHeight - 1, 32, 32);
        context.verticalLine(gameX + cookiePanelWidth, gameY, gameY + gameHeight, -1);

        context.centeredText(font, Component.translatable("minigame.cookieclicker.shop").withStyle(style -> style.withBold(true)), gameX + cookiePanelWidth + upgradePanelWidth / 2, gameY + 11, -1);
