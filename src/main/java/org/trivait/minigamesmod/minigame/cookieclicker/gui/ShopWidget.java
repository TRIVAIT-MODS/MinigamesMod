...
public class ShopWidget extends ClickableWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, BACKGROUND_TEXTURE, getX(), getY(), getWidth(), getHeight());
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (this.active && this.visible&&isMouseOverEntries(mouseX, mouseY)) {
            if (this.isValidClickButton(click.buttonInfo())) {
                ...
                if (bl) {
                    MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.ui(sound, 1, CookieClickerScreen.vol()));
    ...
    public class ShopEntry {
        ...
        public void render(DrawContext ctx, int x, int y, int width, int height, CookieClickerScreen screen, int mouseX, int mouseY) {
            ...
            this.width = width;
            this.height = height;
            ctx.drawGuiTexture(RenderPipelines.GUI_TEXTURED, SHOP_ENTRY_TEXTURE, x, y, width, height);
            ctx.drawTexture(RenderPipelines.GUI_TEXTURED, texture, x, y, 0, 0, height, height, height, height);
            ...
            ctx.getMatrices().translate(x + height + 5, nameY);
            ctx.getMatrices().scale(textScale, textScale);
            ctx.drawText(textRenderer, name, 0, 0, -1, true);
            ...
            ctx.getMatrices().popMatrix();
            ctx.getMatrices().pushMatrix();
            ctx.getMatrices().translate(x + height + 5, priceY);
            ctx.getMatrices().scale(textScale, textScale);
        ...
        public void renderTooltip(DrawContext ctx, int mouseY) {
            ...
            ctx.drawGuiTexture(RenderPipelines.GUI_TEXTURED, BACKGROUND_TEXTURE, startX, startY, tooltipWidth, tooltipHeight);
            ...
            if (!description.isEmpty()) {
                ...
                ctx.getMatrices().pushMatrix();
                ctx.getMatrices().translate(textX, currentY);
                ctx.getMatrices().scale(textScale, textScale);
            ...
            if (!percentText.isEmpty()) {
                ...
                ctx.getMatrices().pushMatrix();
                ctx.getMatrices().translate(textX, currentY + 2);
                ctx.getMatrices().scale(textScale, textScale);
