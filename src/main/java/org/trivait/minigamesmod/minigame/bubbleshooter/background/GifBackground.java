...
public class GifBackground extends Background {
    private static final Identifier MOD_ID = Identifier.fromNamespaceAndPath("minigamesmod", "bubbleshooter_gif");
    ...
    private void loadGif() {
        loaded = true;
        File gifFile = new File(Minecraft.getInstance().gameDirectory, "config/minigames/bubbleshooter.gif");
        if (!gifFile.exists()) {
            return;
        }
            ...
            for (int i = 0; i < count; i++) {
                ...
                for (int y = 0; y < gifHeight; y++) {
                    for (int x = 0; x < gifWidth; x++) {
                        ...
                        int abgr = (a << 24) | (b << 16) | (gChannel << 8) | r;
                        nativeImage.setPixelABGR(x, y, abgr);
                    }
                }

                Identifier frameId = Identifier.fromNamespaceAndPath(
                        MOD_ID.getNamespace(),
                        MOD_ID.getPath() + "_frame_" + i
                );
    ...
    public void render(int x, int y, int width, int height, GuiGraphicsExtractor ctx, float delta, int mouseX, int mouseY) {
        ...
        ctx.blit(RenderPipelines.GUI_TEXTURED, currentFrameId, x, y, 0, 0, width, height, width, height);
    }
