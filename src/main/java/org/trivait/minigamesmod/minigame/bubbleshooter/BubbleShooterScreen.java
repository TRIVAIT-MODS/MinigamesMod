...
public class BubbleShooterScreen extends Screen {
    ...
    private static final Identifier GUI_TEXTURE = Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "textures/minigame/bubble_shooter/gui.png");
    private static final Identifier BUBBLE_TEXTURE = Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "textures/minigame/bubble_shooter/bubble.png");
    ...
    @Override
    protected void init() {
        Button returnButton = SpriteIconButton.builder(Component.empty(), b -> onClose(), true)
                .sprite(Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "icon/return"), 15, 15)
                .build();
        returnButton.setTooltip(Tooltip.create(Component.translatable("minigame.2048.undo")));
        returnButton.setRectangle(20, 20, 10, 10);

        Button restartButton = SpriteIconButton.builder(Component.empty(), b -> resetGame(), true)
                .sprite(Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "icon/restart"), 15, 15)
                .build();
        ...
        addRenderableWidget(new ConfigButton(60, 10, minigame));
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        if (!gameOver) {
            ...
            for (var child : children()) {
                if (child instanceof Button buttonWidget && buttonWidget.isMouseOver(click.x(), click.y())) {
                    overButton = true;
                    break;
                }
            }
    ...
    private void shoot() {
        ...
        PlayingSoundManager.playSound(SoundEvents.CRAFTER_CRAFT, 1, vol());
    }
    ...
    private void updateProjectile(float dt) {
        ...
        while (remaining > 0 && projectileActive) {
            ...
            if (nx < left) {
                ...
                PlayingSoundManager.playSound(SoundEvents.WOODEN_BUTTON_CLICK_ON, 1.5f, vol()/1.5f);
                continue;
            }
            ...
            if (topHit >= 0 && topHit <= step) {
                ...
                PlayingSoundManager.playSound(SoundEvents.WOODEN_BUTTON_CLICK_ON, 1f, vol());
                lockProjectile(-1, -1);
                return;
            }
    ...
    private void resolveMatches(int col, int row) {
        ...
        PlayingSoundManager.playSound(SoundEvents.DEEPSLATE_BREAK, 1.2f, vol()*1.5f);
    ...
    private void removeFloatingBubbles() {
        ...
        if (wave > 0) {
            PlayingSoundManager.playSound(SoundEvents.CHICKEN_EGG, 1.6f, vol());
        }
    }
    ...
    private void drawBoard(GuiGraphicsExtractor context) {
        for (int row = 0; row < GRID_ROWS; row++) {
            for (int col = 0; col < GRID_COLS; col++) {
                if (board[col][row] == 0) {
                    continue;
    ...
    private void drawEffects(GuiGraphicsExtractor context) {
    ...
    private void drawAimer(GuiGraphicsExtractor context) {
    ...
    private void drawQueuedBubble(GuiGraphicsExtractor context) {
    ...
    private void drawTrajectory(GuiGraphicsExtractor context) {
    ...
    private void drawTrajectorySegment(GuiGraphicsExtractor context, float x, float y, float dx, float dy, float length) {
    ...
    private void drawLossLine(GuiGraphicsExtractor context) {
        int y = Math.round(getLossLineY());

        context.fill(getGameLeft(), y, getGameLeft() + GAME_WIDTH, y + 1, 0xFFFF3030);
    }

    private void drawScore(GuiGraphicsExtractor context) {
        Component scoreText = Component.translatable("minigame.bubbleshooter.score").append(""+score);
        Component highText = Component.translatable("minigame.bubbleshooter.highScore").append(""+getHighScore());

        int center = getGameLeft() + GAME_WIDTH / 2;

        context.text(font, scoreText, center - font.width(scoreText) / 2, getGameTop() - 28-1, 0xFFFFFFFF, true);

        context.text(font, highText, center - font.width(highText) / 2, getGameTop() - 16-1, 0xFFFFD95A, true);
    }

    private void drawGameOver(GuiGraphicsExtractor context) {
        Component defeat = Component.translatable("minigame.bubbleshooter.gameOver").copy().withStyle(s -> s.withBold(true));
        ...
        context.text(font, defeat, -font.width(defeat) / 2, -font.lineHeight / 2, 0xFFFF4040, true);

        matrices.popMatrix();
    }

    private void drawBubble(GuiGraphicsExtractor context, float x, float y, int size, int color, float scale) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, BUBBLE_TEXTURE, -renderSize / 2, -renderSize / 2, 0, 0, renderSize, renderSize, renderSize, renderSize, color);
