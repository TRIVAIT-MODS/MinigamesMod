...
public class ScoreSelectWidget extends AbstractWidget {
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.blit(RenderPipelines.GUI_TEXTURED, Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "textures/minigame/minesweeper/score.png"), getX() + 1, getY() + 1, 0, 0, width - 2, height - 2, width - 2, height - 2);

        Minecraft mc = Minecraft.getInstance();
        MutableComponent label = Component.translatable("minigame.minesweeper.leaderboard.mode.score").withStyle(s -> s.withBold(true));
