...
public final class SolitaireScreen extends Screen {
    ...
    protected void init() {
        ...
        layout();
        clearWidgets();
        Button back = SpriteIconButton.builder(Component.empty(), button -> onClose(), true)
                .sprite(Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "icon/return"), 15, 15).build();
        back.setTooltip(Tooltip.create(Component.translatable("minigame.2048.undo")));
        back.setRectangle(20, 20, 10, 10);
        Button restart = SpriteIconButton.builder(Component.empty(), button -> restart(), true)
                .sprite(Identifier.fromNamespaceAndPath(MinigamesMod.MOD_ID, "icon/restart"), 15, 15).build();
        restart.setTooltip(Tooltip.create(Component.translatable("minigame.restart")));
        restart.setRectangle(20, 20, 35, 10);
    ...
    private void startGame() {
        ...
        setupDeal();
        playSound(SoundEvents.WOOL_PLACE, 1.2f);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
            ...
            if (inside(x, y, wasteX(), topY())) {
                if (!game.waste.isEmpty()) {
                    ...
                    if (target != -1 && game.moveWasteToFoundation(target)) {
                        ...
                        playSound(SoundEvents.WOOL_PLACE, 1.1f);
                        clearSelection();
                        checkWon();
                        return true;
                    }
        ...
        if (inside(x, y, stockX(), topY())) {
            ...
            if (game.stock.isEmpty() && !game.waste.isEmpty()) {
                ...
                game.draw();
                playSound(SoundEvents.BUNDLE_DROP_CONTENTS, 1.1f);
            } else if (!game.stock.isEmpty()) {
                game.draw();
                drawAnimActive = true;
                drawStartTime = now;
                drawAnimCard = game.waste.get(game.waste.size() - 1);
                playSound(SoundEvents.WOOL_PLACE, 1.3f);
            }
            clearSelection();
            return true;
        }
        ...
        if (inside(x, y, wasteX(), topY())) {
            if (!game.waste.isEmpty()) {
                ...
                if (selectedWaste) {
                    ...
                    clearSelection();
                    selectedWaste = true;
                    playSound(SoundEvents.WOODEN_BUTTON_CLICK_ON, 1.8f);
                }
            }
            return true;
        }
            ...
            if (index >= 0) {
                ...
                if (!card.faceUp) {
                    if (index == game.tableau.get(column).size() - 1) {
                        game.flip(column);
                        playSound(SoundEvents.WOODEN_BUTTON_CLICK_ON, 1.5f);
                    }
    ...
    private void dropTableau(int target) {
        ...
        if (moved) {
            ...
            playSound(SoundEvents.WOODEN_BUTTON_CLICK_ON, 1.0f);
        }
        clearSelection();
    }
    ...
    public boolean mouseDragged(MouseButtonEvent click, double offsetX, double offsetY) {
        if (click.button() == InputConstants.MOUSE_BUTTON_LEFT && selected()) {
            dragging = true;
            dragX = (int) click.x();
            dragY = (int) click.y();
            return true;
    ...
    public boolean mouseReleased(MouseButtonEvent click) {
        if (click.button() == InputConstants.MOUSE_BUTTON_LEFT && dragging) {
            int x = (int) click.x();
            int y = (int) click.y();
            int column = columnAt(x);
    ...
    public boolean keyPressed(KeyEvent input) {
        ...
        if (input.key() == InputConstants.KEY_Z && (input.modifiers() & InputConstants.MOD_CONTROL) != 0) {
            if (dealing) {
                dealing = false;
            }
            if (game.undo()) {
                ...
                recycleAnimActive = false;
                autoMoveActive = false;
                playSound(SoundEvents.WOODEN_BUTTON_CLICK_ON, 1.3f);
            }
            return true;
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, SolitaireTextures.GUI_TEXTURE, boardLeft, boardTop, 0, 0, BOARD_WIDTH, BOARD_HEIGHT, BOARD_WIDTH, BOARD_HEIGHT);
            ...
            for (DealStep step : dealSteps) {
                if (elapsed >= step.startMs && elapsed < step.startMs + DEAL_DURATION) {
                    ...
                    if (!step.soundPlayed) {
                        ...
                        playSound(SoundEvents.WOOL_PLACE, 0.9f + step.index * 0.05f);
                    }
        ...
        if (game.won) {
            Component winText = Component.translatable("minigame.solitaire.win").withStyle(ChatFormatting.GOLD, ChatFormatting.BOLD);
            int winWidth = font.width(winText);
            context.text(font, winText, boardLeft + (BOARD_WIDTH - winWidth) / 2, boardTop + BOARD_HEIGHT - 18, 0xFFFFD700, true);
        }
    }

    private void drawDragged(GuiGraphicsExtractor context) {
        if (selectedWaste) {
    ...
    private void slot(GuiGraphicsExtractor context, int x, int y) {
        texture(context, SolitaireTextures.OVERLAY_CARD, x, y);
    }

    private void card(GuiGraphicsExtractor context, SolitaireGame.Card card, int x, int y) {
        texture(context, SolitaireTextures.cardTexture(card.suit, card.rank), x, y);
    }

    private void texture(GuiGraphicsExtractor context, Identifier texture, int x, int y) {
        context.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, 0, 0, CARD_WIDTH, CARD_HEIGHT, CARD_WIDTH, CARD_HEIGHT);
    }
