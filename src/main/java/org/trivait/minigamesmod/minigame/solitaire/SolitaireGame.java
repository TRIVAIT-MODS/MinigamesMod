...
public final class SolitaireGame {
    ...
    public void reset() {
        ...
        waste.clear();
        undo.clear();
        java.util.Arrays.fill(foundations, 0);
        java.util.Arrays.fill(foundationSuits, -1);
        won = false;
        List<Card> deck = new ArrayList<>(52);
        for (Suit suit : Suit.values()) {
            for (int rank = 1; rank <= 13; rank++) deck.add(new Card(suit, rank));
        }
    ...
    public boolean flip(int column) {
        ...
        if (pile.isEmpty() || pile.get(pile.size() - 1).faceUp) return false;
        save();
    ...
    public boolean moveWasteToTableau(int column) {
        if (waste.isEmpty() || !canPlaceOnTableau(waste.get(waste.size() - 1), column)) return false;
        save();
    ...
    public boolean moveWasteToFoundation() {
        return waste.isEmpty() ? false : moveWasteToFoundation(waste.get(waste.size() - 1).suit.ordinal());
    }
    ...
    public boolean moveTableauToFoundation(int column) {
        List<Card> pile = tableau.get(column);
        return pile.isEmpty() ? false : moveTableauToFoundation(column, pile.get(pile.size() - 1).suit.ordinal());
    }
    ...
    public boolean moveTableauToFoundation(int column, int foundation) {
        ...
        if (!card.faceUp || !canPlaceOnFoundation(card, foundation)) return false;
        save();
    ...
    public boolean moveTableauToTableau(int from, int cardIndex, int to) {
        ...
        if (cardIndex < 0 || cardIndex >= source.size() || !canMoveRun(source, cardIndex) || !canPlaceOnTableau(source.get(cardIndex), to)) return false;
        save();
    ...
    public boolean moveTableauToWaste(int column) {
        List<Card> pile = tableau.get(column);
        if (pile.isEmpty()) return false;
        Card card = pile.get(pile.size() - 1);
        if (!card.faceUp) return false;
        save();
        waste.add(pile.remove(pile.size() - 1));
        flipAfterMove(pile);
        return true;
    }

    public boolean moveFoundationToTableau(int suit, int column) {
        int rank = foundations[suit];
        if (rank == 0) return false;
        Card card = new Card(Suit.values()[foundationSuits[suit]], rank);
        card.faceUp = true;
        if (!canPlaceOnTableau(card, column)) return false;
        save();
        foundations[suit]--;
        if (foundations[suit] == 0) foundationSuits[suit] = -1;
    ...
    public boolean canMoveRun(List<Card> pile, int index) {
        if (index < 0 || index >= pile.size() || !pile.get(index).faceUp) return false;
        for (int i = index + 1; i < pile.size(); i++) {
            Card upper = pile.get(i - 1);
            Card lower = pile.get(i);
            if (!lower.faceUp || upper.rank != lower.rank + 1 || upper.isRed() == lower.isRed()) return false;
        }
        return true;
    }
    ...
    private boolean canPlaceOnFoundation(Card card) {
        return foundations[card.suit.ordinal()] + 1 == card.rank;
    }

    private boolean canPlaceOnFoundation(Card card, int foundation) {
        return foundation >= 0 && foundation < 4
                && foundations[foundation] + 1 == card.rank
                && (foundations[foundation] == 0 || foundationSuits[foundation] == card.suit.ordinal());
    }

    private void flipAfterMove(List<Card> pile) {
        if (!pile.isEmpty() && !pile.get(pile.size() - 1).faceUp) pile.get(pile.size() - 1).faceUp = true;
    }
    ...
    private void restore(State state) {
        ...
        stock.clear();
        waste.clear();
        for (List<Card> pile : state.tableau) tableau.add(copyPile(pile));
    ...
    private static final class State {
        ...
        private State(SolitaireGame game) {
            for (List<Card> pile : game.tableau) tableau.add(copyPile(pile));
