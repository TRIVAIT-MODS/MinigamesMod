...
public class MainConfigButton extends SpriteIconButton.CenteredIcon {
    ...
    public MainConfigButton(int x, int y) {
        super(20, 20, Component.empty(), 20, 20, 0, 0, new WidgetSprites(DEFAULT_TEXTURE, HOVER_TEXTURE), button -> {
            Minecraft client = Minecraft.getInstance();
            Screen screen = client.gui.screen();
            screen.onClose();
            client.gui.setScreen(AutoConfigClient.getConfigScreen(Config.class, screen).get());
